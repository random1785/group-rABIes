...
execute as @e[tag=score] run execute unless score @s gone matches 1 run execute as @a[tag=!a] run tellraw @a [{"selector":"@s","color":"dark_red"},{"text":" has ran out of time, ending with a score of 2400."}]
...
execute as @e[tag=score] run scoreboard players set @a[scores={done=0},tag=!a] calc 2400
execute as @e[tag=score] run scoreboard players set @s scoretick 2400